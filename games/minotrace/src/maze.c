...
// Maps maze field type (>>2) to display character for minimap
static const char mazelut[] = {
	' ', '#', '*', ' ',
	'#', '#', '#', '#'
};

// Color lookup for maze preview minimap
static const char mazecolorlut[] = {
	0x00, 0xF0, 0x20, 0x00,
	0x20, 0x60, 0x40, 0xF0
};
...
{
	// Switch to autoscroll then user scroll mode
	...
	for(;;)
	{
		if (autoscroll)
		{
			// Autoscroll by one column per frame
			...
			if (mx < (int)maze_size)
			{
				for(char j=0; j<25; j++)
				{
					...
					char ch = mazelut[p >> 2];
					char co = mazecolorlut[p >> 2];
					screen_chars[40 * j + 39] = ch;
					screen_colors[40 * j + 39] = co;
				}
			}
			else
			{
				for(char j=0; j<25; j++)
				{
					screen_chars[40 * j + 39] = ' ';
					screen_colors[40 * j + 39] = 0x00;
				}
			}
		...
		if (joyb && !down)
		{
			// Button was up and is now down, exit preview
			return;
		}
		else if (joyx < 0)
		{
			// End auto scroll, scroll left manually
			autoscroll = false;
			...
			if (mx >= 39)
			{
				for(char j=0; j<25; j++)
				{
					...
					screen_chars[40 * j] = mazelut[p >> 2];
					screen_colors[40 * j] = mazecolorlut[p >> 2];
				}
			}
			else
			{
		...
		else if (joyx > 0)
		{
			...
			if (mx < (int)maze_size)
			{
				...
				{
					char p = maze_grid[256 * j + mx];
					screen_chars[40 * j + 39] = mazelut[p >> 2];
					screen_colors[40 * j + 39] = mazecolorlut[p >> 2];
				}
			}
			else
			{
...
void maze_build(const MazeInfo * info)
{
	...
	// Store colors for the display system
	display_wall_color = info->colors >> 4;
	display_floor_color = info->colors & 0x0f;
}
